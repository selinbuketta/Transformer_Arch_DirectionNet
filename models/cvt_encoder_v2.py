...
Practical CVT tuning hints for ~1k samples on midrange GPUs (4050):
- Start narrower/shallower to reduce overfitting and VRAM (e.g., `embed_dim=192`,
  `num_layers=2`, `num_heads=4`, `mlp_ratio=3.0`, `dropout_rate=0.1`).
- Use slightly larger `patch_size` (24–32) to lower token count and attention cost.
- Increase L2 on the patch embed (e.g., `regularization=0.02–0.05`) when data is scarce.
- Lightweight positional cues are built in via sinusoidal embeddings (no extra params);
  leave enabled to help spatial reasoning on small datasets.
...
class MultiHeadSelfAttention(Layer):
  ...
  def _ensure_layers(self):
    """Lazily recreate sub-layers if a deserialized object missed them."""
    # During graph deserialization or object reuse, Keras might not restore
    # custom attributes. Guard against missing layers to avoid attribute errors.
...
class CrossViewTransformerEncoder(keras.Model):
  ...
  def _sinusoidal_positional(self, H, W):
    # Build a 2D sinusoidal embedding with separate freq banks for y/x.
    y = tf.cast(tf.range(H), tf.float32)
    x = tf.cast(tf.range(W), tf.float32)
    yy, xx = tf.meshgrid(y, x, indexing='ij')  # [H,W]
    coords = tf.reshape(tf.stack([yy, xx], axis=-1), [H * W, 2])  # [T,2]

    half = self.embed_dim // 2
    # Use even positions for frequencies; if embed_dim not divisible by 4, pad later.
    freq_range = tf.range(0, half, 2, dtype=tf.float32)
    inv_freq = tf.exp(-tf.math.log(10000.0) * freq_range / tf.cast(tf.maximum(1, half), tf.float32))

    def _encode(coord):
      # coord: [T,1]
      sinusoid_inp = coord * inv_freq  # [T, F]
      return tf.concat([tf.sin(sinusoid_inp), tf.cos(sinusoid_inp)], axis=-1)  # [T, 2F]

    pos_y = _encode(coords[:, 0:1])
    pos_x = _encode(coords[:, 1:2])
    pos = tf.concat([pos_y, pos_x], axis=-1)  # [T, <=embed_dim]
    # Pad or trim deterministically to match embed_dim to stay drop-in compatible.
  ...
  def _add_positional(self, tokens, H, W, training):
    ...
    pos = tf.expand_dims(tf.cast(pos, tokens.dtype), axis=0)  # [1,T,C]
    return tokens + pos
