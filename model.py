...
try:
  from models.cvt_encoder_main import CrossViewTransformerEncoder
except Exception:
  CrossViewTransformerEncoder = None

# Optional: Coarse Transformer encoder
try:
  from models.coarse_transformer_encoder import CoarseTransformerEncoder
except Exception:
  CoarseTransformerEncoder = None
...
class DirectionNet(keras.Model):
  ...
  def __init__(self, n_out, regularization=0.01, encoder_type='siamese'):
    """Initialize the DirectionNet.

    Args:
      n_out: (int) the number of output distributions.
      regularization: L2 regularization factor for layer weights.
      encoder_type: 'siamese' (default) or 'cvt' for Cross-View Transformer.
    """
    ...
    elif encoder_type in ['coarse_transformer', 'coarse'] and \
      CoarseTransformerEncoder is not None:
      self.encoder = CoarseTransformerEncoder(regularization=regularization)
    else:
